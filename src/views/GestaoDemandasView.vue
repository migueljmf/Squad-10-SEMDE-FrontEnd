      ...
      <template v-else>
        ...
        >
          ...
          <div class="demanda-meta">
            ...
            <span>{{ demanda.localFormatado }}</span>
...
const demandasFormatadas = computed(() =>
  demandasFiltradas.value.map((item) => {
    ...
    const candidatosLocal = [
      typeof item.local === "string" ? item.local.trim() : "",
      typeof item.enderecoResumo === "string" ? item.enderecoResumo.trim() : "",
      typeof item.endereco === "string" ? item.endereco.trim() : "",
    ];
    const localFormatado = candidatosLocal.find((valor) => valor) || "Local nao informado";
    return {
      ...item,
